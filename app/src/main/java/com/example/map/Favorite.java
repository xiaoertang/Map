 ...
 * 演示如何使用本地点收藏功能
 */
public class Favorite extends AppCompatActivity implements OnMapLongClickListener, OnMarkerClickListener,
        OnMapClickListener {
...
//初始化定位
    ...
    public void saveClick(View v) {
        if (mEditName.getText().toString() == null || mEditName.getText().toString().equals("")) {
            Toast.makeText(Favorite.this, "名称必填", Toast.LENGTH_LONG).show();
            return;
        }
        if (mEditLocation.getText().toString() == null || mEditLocation.getText().toString().equals("")) {
            Toast.makeText(Favorite.this, "坐标点必填", Toast.LENGTH_LONG).show();
            return;
        }
        ...
        try {
            ...
            if (FavoriteManager.getInstance().add(info) == 1) {
                Toast.makeText(Favorite.this, "添加成功", Toast.LENGTH_LONG).show();
            } else {
                Toast.makeText(Favorite.this, "添加失败", Toast.LENGTH_LONG).show();
                return;
            }

        } catch (Exception e) {
            Toast.makeText(Favorite.this, "坐标解析错误", Toast.LENGTH_LONG).show();
            return;
        }
    ...
    public void modifyClick(View v) {
        ...
        if (null == currentID) {
            Toast.makeText(Favorite.this, "该点未收藏，无法修改", Toast.LENGTH_LONG).show();
            return;
        }

        if (null == FavoriteManager.getInstance().getFavPoi(currentID)) {
            Toast.makeText(Favorite.this, "获取Poi失败", Toast.LENGTH_LONG).show();
            return;
        }
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (newName != null && !newName.equals("")) {
                    ...
                    if (FavoriteManager.getInstance().updateFavPoi(currentID, info)) {
                        Toast.makeText(Favorite.this, "修改成功", Toast.LENGTH_LONG).show();
                    }
                } else {
                    Toast.makeText(Favorite.this, "名称不能为空，修改失败", Toast.LENGTH_LONG).show();
                }
                dialog.dismiss();
            }
    ...
    public void deleteOneClick(View v) {
        ...
        if (null == currentID) {
            Toast.makeText(Favorite.this, "该点未收藏，无法进行删除操作", Toast.LENGTH_LONG).show();
            return;
        }

        if (FavoriteManager.getInstance().deleteFavPoi(currentID)) {
            Toast.makeText(Favorite.this, "删除点成功", Toast.LENGTH_LONG).show();
            if (markers != null) {
            ...
            Toast.makeText(Favorite.this, "删除点失败", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void getAllClick(View v) {
        ...
        if (list == null || list.size() == 0) {
            Toast.makeText(Favorite.this, "没有收藏点", Toast.LENGTH_LONG).show();
            return;
        }
        // 绘制在地图
        markers.clear();
    ...
    public void deleteAllClick(View v) {
        if (FavoriteManager.getInstance().clearAllFavPois()) {
            Toast.makeText(Favorite.this, "全部删除成功", Toast.LENGTH_LONG).show();
            mBaiduMap.clear();
            mBaiduMap.hideInfoWindow();
        } else {
            Toast.makeText(Favorite.this, "全部删除失败", Toast.LENGTH_LONG).show();
        }
    }
